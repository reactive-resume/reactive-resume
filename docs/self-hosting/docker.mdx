...
Multiple application instances should share `REDIS_URL` and `DEPLOYMENT_NAMESPACE` for rate limits, resumable streams, cancellation, resume notifications, and view deduplication. Docker without Redis retains its existing core resume features. Private agent attachments still need S3-compatible storage or private Blob; local disk supports ordinary public uploads.
