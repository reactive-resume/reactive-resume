...
If you cloned the Reactive Resume repository, you can use the included startup helper to start the Docker Compose stack and open the application automatically:

```bash
./scripts/self-hosting/start.sh
```
The helper waits for the Docker Compose services to become ready before opening the application in your default browser. The timeout can be configured with the `TIMEOUT_SECONDS` environment variable.

For example:

```bash
TIMEOUT_SECONDS=300 ./scripts/self-hosting/start.sh
```
